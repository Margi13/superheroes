...
export const generateResponseDocument = ( document ) => {
...
const createMainText = (document) => {
    const text = [
        ...
        `СОБСТВЕНИКЪТ ${document.response === true ? "прие" : "отказа"} молбата на ПОДАТЕЛЯ за използване на неговия ${document.requestedDataType === "comics" ? "комикс" : "герой"} с ИН: ${document._dataId} за използването му в нов ${document.newDataType === "comics" ? "комикс" : "герой"}, на базата на изпратеното съобщение от ПОДАТЕЛЯ:`,
    ];
